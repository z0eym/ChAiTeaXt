...
{/* Start Now Button - Logic Aware */}
<button
  onClick={() => {
    if (user) {
      startNewChat(); // If logged in, go to a new chat
    } else {
      router.push("/signup"); // If guest, go to signup
    }
  }}
  className="cursor-pointer gap-4 mt-6 text-2xl text-white flex h-16 items-center justify-center rounded-full bg-black/70 transition-colors hover:bg-[#5e17eb] dark:hover:bg-[#ae97e7] px-8"
> 
  {user ? "Launch Chat" : "Get Started"}
</button>
    </div>
